...
static func create_physical_bone(parent:Node3D,skeleton:Skeleton3D,bone_name1:String,bone_name2:String,shape_height_multiply:float = 1,radius_ratio:float = 0.25,rotating = true,same_bone_offset = Vector3(0,-0.02,0),shape_location_ratio = 0.5)-> PhysicalBone3D:	
	...
	#physical_bone.set_name("PhysicalBone " + bone_name1 + " - " + bone_name2)
	
	physical_bone.joint_type = 1 # pin
	
	physical_bone.bone_name = bone_name1 
	parent.add_child(physical_bone)
	physical_bone.set_name("PhysicalBone " + bone_name1)
	#print(physical_bone.get_name())
	...
	#print("%s = global %s rest = %s"%[bone_name1,rot1,rest])
	
	var origin1 = bone_transform1.origin
	var origin2 = bone_transform2.origin
	
	if bone_name1 == bone_name2:
		#print("add same bone deprecated ")
		origin2 += same_bone_offset
	
	var global_diff_pos = origin2 - origin1
	
	
	#var diff_tr1=bone_transform1.translated(global_diff_pos)
	#var newt2 = bone_transform1.looking_at(diff_tr1.origin,Vector3.UP)
	
	#physical_bone.look_at(origin2)
	...
	#var diff2 = origin1 - diff_tr1.origin
	
	#if bone_name1 == bone_name2:
		#I'M not sure
	#	cshape.global_transform .origin = physical_bone.global_transform .origin
	#	physical_bone.global_transform .origin = origin1 + global_diff_pos/2*height_multiply
	#else:
